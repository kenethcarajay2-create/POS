    ...
    */
    ...
    const addCredit = async (
        ...
        if (
            ...
            throw new ApiError(
                400,
                "At least one credit item is required."
            );
        ...
        */
        ...
        for (
            ...
            If productId exists, treat as PRODUCT.

            If productId does not exist, CUSTOM can be used.
            ====================================================
            */
        ...
        Prevent credits from exceeding remaining worker salary.
        ========================================================
        */
        ...
        if (
            ...
            if (
                ...
                throw new ApiError(
                    400,
                    `Credit cannot exceed the worker's remaining salary of ${remainingSalary}.`
                );
    ...
    */
    ...
    /*
============================================================
REBUILD WORKER LEDGER TOTALS
============================================================

Recalculates the WorkerLedger from the transactions that
actually belong to that ledger.

This is safer than manually subtracting the old transaction
and adding the edited transaction.

IMPORTANT:

Credit payments do NOT restore worker salary.
...
const rebuildWorkerLedgerTotals = async (
    workerLedger
) => {
...
/*
============================================================
RETURN OLD CREDIT INVENTORY
============================================================

Before editing a CREDIT transaction we return the stock
used by the ORIGINAL transaction.

CUSTOM items are ignored because they do not affect stock.
============================================================
*/

const reverseOldCreditInventory = async (
    transaction,
    userId,
    account
) => {
    ...
    for (
        ...
        */
        ...
        if (!product) {
            ...
            throw new ApiError(
                404,
                `Original product "${item.name}" no longer exists.`
            );
        ...
        */
        ...
        await InventoryTransaction.create({
            ...
            remarks:
                `Admin edit: reversed old ledger credit for ${account.name}`,
...
/*
============================================================
PREPARE EDITED CREDIT ITEMS
============================================================
*/

const prepareEditedCreditItems = async (
    items
) => {
    ...
    for (
        ...
        */
        ...
        The OLD transaction inventory has already been
        returned before this function is called.

        Therefore current product.stock represents the stock
        available BEFORE applying the corrected transaction.
        */
    ...
    for (
        ...
        */
        ...
        await InventoryTransaction.create({
            ...
            remarks:
                `Admin edit: applied corrected ledger credit for ${account.name}`,
...
The route must already be protected using:
...
Worker transactions can only be edited while their
WorkerLedger is OPEN.

PAID/CLOSED worker ledgers are historical records and
cannot be changed.
============================================================
*/
...
const updateTransaction = async (
    ...
    */
    ...
    const transaction =
        await LedgerTransaction.findOne({
    ...
    */
    ...
    if (
        transaction.workerLedger
    ) {

        workerLedger =
            await WorkerLedger.findById(
                transaction.workerLedger
            );
    ...
    */
    ...
    if (
        ...
        */
        ...
        await reverseOldCreditInventory(
            transaction,
            adminUserId,
            account
        );
        ...
        try {
            ...
            This also validates stock using the restored
            inventory quantities.
            =================================================
            */

            prepared =
                await prepareEditedCreditItems(
                    data.items
                );
            ...
            /*
            =================================================
            WORKER SALARY VALIDATION
            =================================================

            The OLD transaction is still included in the
            worker ledger totals at this point.

            So calculate:

            salary
            - (current credits - old amount + new amount)
            - cash advances
            =================================================
            */
            ...
            if (
                ...
                if (
                    ...
                    throw new ApiError(
                        400,
                        "The corrected credit exceeds the worker's available salary."
                    );
            ...
            */
            ...
            const transactionItems =
                await applyEditedCreditInventory(
            ...
            */
            ...
            If validation of the NEW transaction fails after
            the original stock was returned, we need to put
            the ORIGINAL stock effect back.

            Otherwise a failed edit would incorrectly increase
            inventory.
            =================================================
            */
    ...
    */
        ...
        /*
        ====================================================
        PAYMENT CANNOT EXCEED TOTAL CREDIT

        Exclude current payment before calculating available
        outstanding balance.
        ====================================================
        */
        ...
        if (
            ...
            if (
                ...
                throw new ApiError(
                    400,
                    `Payment cannot exceed the outstanding balance of ${availableOutstanding}.`
                );
    ...
    */
        ...
        if (
            ...
            throw new ApiError(
                400,
                "Cash advance transaction is not attached to a worker ledger."
            );
        ...
        Remove old cash advance then apply new amount.
        ====================================================
        */

        const projectedCashAdvances =
            Math.max(
        ...
        if (
            ...
            throw new ApiError(
                400,
                "The corrected cash advance exceeds the worker's remaining salary."
            );
    ...
    */
        ...
        throw new ApiError(
            400,
            "This transaction type cannot be edited."
        );
    ...
    */
    ...
    if (
        ...
        await rebuildWorkerLedgerTotals(
            workerLedger
        );
